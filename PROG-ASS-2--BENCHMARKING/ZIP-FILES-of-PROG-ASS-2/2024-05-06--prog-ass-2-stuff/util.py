...
def build_query_reviews_no_index(word, date1, date2):
    q41 = """SELECT comments, comments_tsv
FROM reviews r 
...
def build_query_reviews_ts(word, date1, date2):
    q51 = """SELECT comments, comments_tsv
FROM reviews r 
...
END TRANSACTION;
"""
            modify_index = q1 + index_name + q2 + i_spec[1] + q3 + i_spec[0] + q4
...
END TRANSACTION;
    """
            ...
            print('ERROR: call to function add_drop_index has invalid i_spec value:', str(i_spec))
...
def fetch_and_update_perf_data(perf_file, new_perf_summary):
...
def run_one_text_query_without_with_tsindex(db_eng, word, date1, date2, count):
    ...
    output[key] = {}
    output[key]['no_index'] = perf_profile1
...
def fetch_and_update_ts_perf_data(perf_file, new_perf_summary):
    # fetch or create file perf_data/perf_file 
    # print('\nEntering fetch_and_update_perf_data')
    try:
        old_perf_summary = fetch_perf_data(perf_file)
