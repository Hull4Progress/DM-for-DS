...
def build_query_reviews_no_index(word, date1, date2):
    q41 = """SELECT *
FROM reviews r 
...
def build_query_reviews_ts(word, date1, date2):
    q51 = """SELECT *
FROM reviews r 
...
def fetch_and_update_perf_data_merge_for_query(perf_file, new_perf_summary):
...
def run_one_text_query_without_with_tsindex(db_eng, word, date1, date2, count):

    # for each set in subsets we do a run for exactly the i_specs whose counter is in the set

    q1 = build_query_reviews_no_index(word, date1, date2)
    q2 = build_query_reviews_ts(word, date1, date2)
...
def fetch_and_update_ts_perf_data_overwrite_for_query(perf_file, new_perf_summary):
    # fetch or create file perf_data/perf_file
    # print('\nEntering fetch_and_update_perf_data')
    try:
        old_perf_summary = fetch_perf_data(perf_file)
...
def run_one_query_and_multi_index_specs_with_ts(db_eng, q_vec, all_indexes, i_spec_list, count):
    # Build set of i_specs that will be used for a series of tests
    # first, build a list of all subsets of numbers in [0..len(i_spec_list)-1]
    subsets = build_all_subsets(len(i_spec_list))
...
def run_one_text_query_without_with_ts_index(db_eng, word, date1, date2, count):
...
def run_neigh_update_dates_in_reviews_and_multi_index_specs_with_ts(db_eng, neigh, days, all_indexes, i_spec_list, count):
    # Build set of i_specs that will be used for a series of tests
    # first, build a list of all subsets of numbers in [0..len(i_spec_list)-1]
    subsets = build_all_subsets(len(i_spec_list))
    ...
    for set in subsets:
        ...
        print('\In neigh_updates routine, have set add_days to:', str(add_days))
        i_specs = []
        for n in set:
